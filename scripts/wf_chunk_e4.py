#!/usr/bin/env python3
"""Helper para construir mapas de chunk de e1 (clon de wf_chunk.py, target=e1).
Uso en script: from wf_chunk_e1 import O, repl, put, dump
  put(id, full_string)             # registro limpio: string completo
  repl(id, (en,es), (en,es), ...)  # splice: reemplaza dialogo sobre el ORIGINAL (preserva tokens/padding/soup)
  dump(out_path)
O(id) devuelve texto_original."""
import json,os
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_recs=json.load(open(os.path.join(ROOT,"translated_text","e4.json"),encoding="utf-8"))
